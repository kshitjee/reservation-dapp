...
//   res.json(auction)
//   if (auction) {
//     const { bidder, bid, amount } = await req.body;
//     const smol = amount;
...
//     //check if amount is less than available quantity
//     if (amount > auction.quantity - auction.queue.length) {
//       res.status(400);
//       throw new Error("Amount is too high");
//     }
//     else if (bid < auction.minimumThreshold) {
//       res.status(400);
//       throw new Error("Bid is too low");
//     }
//     else if (auction.queue.length < auction.quantity-amount) {
...
//       while (auction.queue.length < auction.quantity) {
//         auction.queue.push({ bidder, bid });
//         auction.leastBid = Math.min(auction.leastBid, bid);
//         auction.currentNumberOfBids ++;
//         smol--;
//       }
//       for(let j=0; j<smol; j++){
